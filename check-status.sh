...
# Usage: ./check-status.sh [development|production]
# Default: development
...
if lsof -Pi :3000 -sTCP:LISTEN -t >/dev/null 2>&1 || netstat -an 2>/dev/null | grep -q ":3000.*LISTEN"; then
    echo -e "${GREEN}[OK]${NC} Frontend server is running on port 3000"
else
...
echo "  ./nginxsetup.sh $MODE - Setup Nginx for $MODE mode"
echo ""
...
echo "  ./check-status.sh development - Check development status"
echo "  ./check-status.sh production  - Check production status"
